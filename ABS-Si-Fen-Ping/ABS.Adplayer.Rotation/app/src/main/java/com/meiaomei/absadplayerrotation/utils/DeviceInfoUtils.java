...
public class DeviceInfoUtils {
    ...
    public static boolean isAppOnForeground() {
        ActivityManager activityManager = (ActivityManager) AbsPlbsApplication.getAppContext().getSystemService(Context.ACTIVITY_SERVICE);
        String packageName = AbsPlbsApplication.getAppContext().getPackageName();
        /**
    ...
    public static void uninstallSlient(String apkName, String taskId, String version, String macAdress, String ipAndPort) {
        String cmd = "pm uninstall " + AbsPlbsApplication.getAppContext().getPackageName();
    ...
    public static String getAppVersionName() {
        ...
        try {
            // 获取PackageManager的实例
            PackageManager packageManager = AbsPlbsApplication.getAppContext().getPackageManager();
            // getPackageName()是当前类的包名，0代表是获取版本信息
            PackageInfo packageInfo = packageManager.getPackageInfo(AbsPlbsApplication.getAppContext().getPackageName(), 0);
            versionName = packageInfo.versionName;
            if (TextUtils.isEmpty(versionName)) {
                return "";
            }
        ...
        try {
            ...
            while ((readLine = bufferedReader.readLine()) != null) {//执行命令cmd，只取结果中含有"HWaddr"的这一行
                if (readLine.contains("HWaddr")) {
                    return readLine.substring(readLine.indexOf("HWaddr") + 7, readLine.length() - 1);
                }
            }
    ...
    /**
     * 以太网下获取mac地址  前提是以太网已经联网了（如果以太网没有联网获取不到这个值）
     * wifi 下同样可以获得 mac地址
     */
    ...
    public  HashMap<String, String> getWebMacInfo() {
        ...
        int appCode = SoftwareUtils.getVersionCode(context);
        Log.e("===appCode", "======" + appCode);//获取app版本
...
//          String s_serverAddress="Server IP: "+Formatter.formatIpAddress(d.serverAddress);//服务器地址
        ...
        map.put("appVersion", String.valueOf(appCode));
