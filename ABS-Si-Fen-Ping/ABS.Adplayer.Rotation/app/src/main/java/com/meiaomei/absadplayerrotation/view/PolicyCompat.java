...
/**
 * Created by meiaomei on 2017/3/9.
 */
public class PolicyCompat {
    ...
    public static Window createWindow(Context context) {
        ...
        if (android.os.Build.VERSION.SDK_INT>=23){//6.0走这个方法
            return makeNewWindow6(context);
        }else {//6.0以下的
            if (false)
     ...
     * @return   phoneWindow
     */
    private static Window makeNewWindow6(Context context) {
        try {
            Class<?> cls = Class.forName(PHONE_WINDOW_CLASS_NAME);
            Constructor<?> localConstructor = cls.getConstructor(new Class[]{Context.class});
            Window window = (Window) localConstructor.newInstance(new Object[]{context});
            return window;
