package com.meiaomei.absadplayerrotation.view.dialog;
...
public class AlertDialogCommon {
    ...
    public  void createAlertDialog(){
        ...
        if (lineNo>16){
            ...
            p.height = (int) (windowsHeight * 0.435); // 高度设置为屏幕的0.3
        }
        windowsWidth = wm.getDefaultDisplay().getWidth();//获取屏幕宽度
        p.width = (int) (windowsWidth * 0.545); // 宽度设置为屏幕宽的
        ...
        if (mBuilder.contents!=null){  //设置内容显示
            ...
            for (int i=0;i<mBuilder.contents.length;i++){
                ...
                if (mBuilder.contents.length !=1){       //只有1条
                    layoutParams.addRule(RelativeLayout.BELOW,(i));
                    layoutParams.topMargin= UnitUtils.dip2px(context, 3); //顶端距离
                }else{//130dp
                    layoutParams.topMargin= UnitUtils.dip2px(context,65);
                    layoutParams.addRule(RelativeLayout.CENTER_HORIZONTAL);
                    layoutParams.addRule(RelativeLayout.CENTER_VERTICAL);
                }
                layoutParams.leftMargin=UnitUtils.dip2px(context,20);//距离左端
                layoutParams.rightMargin=UnitUtils.dip2px(context,20);//距离右端
